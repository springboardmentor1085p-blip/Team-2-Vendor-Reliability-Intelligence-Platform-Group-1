...
import { ApexAxisChartSeries, ApexChart, ApexXAxis, ApexStroke, ApexFill, ApexTooltip, ApexDataLabels, ApexGrid } from 'ng-apexcharts';

export interface QuarterlyTrendItem { quarter: string; amount: number; }

@Component({
  selector: 'app-quarterly-area-chart',
  standalone: true,
  ...
  template: `
    <div class="chart-wrap">
      ...
      <div *ngIf="!loading && !series[0]?.data?.length" class="empty-state">No quarterly data available</div>
    </div>`,
  styles: [`
...
export class QuarterlyAreaChartComponent implements OnChanges {
  @Input() data: QuarterlyTrendItem[] = [];
  @Input() loading = false;

  series: ApexAxisChartSeries = [{ name: 'Quarterly Spend', data: [] }];
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['data'] && this.data) {
      this.series = [{ name: 'Quarterly Spend ($)', data: this.data.map(d => d.amount) }];
      this.xaxis  = { ...this.xaxis, categories: this.data.map(d => d.quarter) };
    }
  }
}
