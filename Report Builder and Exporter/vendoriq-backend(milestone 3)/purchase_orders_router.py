...
def list_purchase_orders(
    status: Optional[str] = Query(None, description="Filter by status: pending | approved | shipped | delivered | cancelled"),
    start_date: Optional[date] = Query(None, description="Lower bound on order_date (YYYY-MM-DD, inclusive)"),
    end_date: Optional[date] = Query(None, description="Upper bound on order_date (YYYY-MM-DD, inclusive)"),
    vendor_id: Optional[int] = Query(None, description="Restrict results to a single vendor"),
    ...
    if filters.start_date:
        query = query.filter(models.PurchaseOrder.order_date >= _day_start(filters.start_date))
    if filters.end_date:
        query = query.filter(models.PurchaseOrder.order_date <= _day_end(filters.end_date))
...
def update_purchase_order(po_id: int, payload: schemas.POUpdate, db: Session = Depends(get_db)):
